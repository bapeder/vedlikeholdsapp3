...
def login():
    ...
    if st.button("Logg inn"):
        conn = sqlite3.connect(DB_FILE, check_same_thread=False)
        volunteer = pd.read_sql_query(f"SELECT id, role FROM volunteers WHERE email = ?", conn, params=(email,))
        conn.close()
...
if page == "Hjem":
    ...
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    tasks = pd.read_sql_query(f"SELECT * FROM tasks WHERE date = ?", conn, params=(str(datetime.date.today()),))
    conn.close()
    if not tasks.empty:
    ...
    if st.button("Legg til vakt"):
        if volunteer != "Ingen frivillige tilgjengelig":
            ...
            conn.commit()
            volunteer_email = pd.read_sql_query(f"SELECT email FROM volunteers WHERE name = ?", conn, params=(volunteer,))["email"].iloc[0]
            conn.close()
    ...
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    tasks = pd.read_sql_query(f"SELECT * FROM tasks WHERE date = ?", conn, params=(str(date),))
    conn.close()
    if not tasks.empty:
    ...
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    user_role = pd.read_sql_query(f"SELECT role FROM volunteers WHERE id = ?", conn, params=(st.session_state.get("volunteer_id", ""),))
    conn.close()
        ...
        if not tasks.empty:
            ...
            if st.button("Oppdater oppgave"):
                ...
                if new_status == "Fullført" and new_volunteer != "TBD":
                    ...
                    volunteer_email = pd.read_sql_query(f"SELECT email FROM volunteers WHERE name = ?", conn, params=(new_volunteer,))["email"].iloc[0]
