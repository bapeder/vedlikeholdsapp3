...
# Prøv å importere streamlit_calendar, men fortsett hvis den ikke er tilgjengelig
try:
...
def init_db():
    try:
        ...
        st.error(f"Feil ved oppretting av database: {str(e)}")
    finally:
        conn.close()
...
def send_email(to_email, subject, body):
    try:
        if not SMTP_EMAIL or not SMTP_PASSWORD or "DIN_GMAIL" in SMTP_EMAIL:
            st.error("SMTP-konfigurasjon mangler. Oppdater SMTP_EMAIL og SMTP_PASSWORD i app.py med gyldig Gmail-adresse og App Password.")
            return False
        msg = MIMEText(body)
        msg["Subject"] = subject
        msg["From"] = SMTP_EMAIL
        ...
        st.success(f"E-post sendt til {to_email}")
        return True
    except smtplib.SMTPAuthenticationError as e:
        st.error(f"SMTP-autentisering feilet: {str(e)}. Sjekk at SMTP_EMAIL og SMTP_PASSWORD er korrekt, og at App Password er gyldig. Generer et nytt App Password om nødvendig.")
        return False
    except smtplib.SMTPConnectError as e:
        st.error(f"Kunne ikke koble til SMTP-server: {str(e)}. Sjekk nettverkstilkobling eller prøv igjen senere.")
        return False
    except smtplib.SMTPException as e:
        st.error(f"SMTP-feil: {str(e)}. Sjekk Gmail-innstillinger eller kontakt administrator.")
        return False
    except Exception as e:
        st.error(f"Uventet feil ved sending av e-post: {str(e)}. Kontakt administrator.")
        return False
...
def login():
    ...
    if st.button("Logg inn"):
        conn = sqlite3.connect(DB_FILE, check_same_thread=False)
        volunteer = pd.read_sql_query("SELECT id, role FROM volunteers WHERE email = ?", conn, params=(email,))
        conn.close()
...
if page == "Hjem":
    ...
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    tasks = pd.read_sql_query("SELECT * FROM tasks WHERE date = ?", conn, params=(str(datetime.date.today()),))
    conn.close()
    if not tasks.empty:
    ...
    if st.button("Legg til vakt"):
        if volunteer != "Ingen frivillige tilgjengelig":
            ...
            conn.commit()
            volunteer_email = pd.read_sql_query("SELECT email FROM volunteers WHERE name = ?", conn, params=(volunteer,))["email"].iloc[0]
            conn.close()
    ...
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    tasks = pd.read_sql_query("SELECT * FROM tasks WHERE date = ?", conn, params=(str(date),))
    conn.close()
    if not tasks.empty:
    ...
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    user_role = pd.read_sql_query("SELECT role FROM volunteers WHERE id = ?", conn, params=(st.session_state.get("volunteer_id", ""),))
    conn.close()
    if not user_role.empty and user_role["role"].iloc[0] == "Administrator":
        ...
        if CALENDAR_AVAILABLE:
            ...
            try:
                calendar(events=events, options={"initialView": "dayGridMonth", "editable": True, "selectable": True})
            except Exception as e:
                st.error(f"Feil i kalendervisning: {str(e)}. Viser tabell i stedet.")
                st.dataframe(tasks[["date", "task", "volunteer", "status", "points"]])
        else:
            st.warning("Kalenderfunksjonalitet er ikke tilgjengelig. Installer streamlit-calendar for å aktivere.")
            st.dataframe(tasks[["date", "task", "volunteer", "status", "points"]])
        ...
        if not tasks.empty:
            ...
            if st.button("Oppdater oppgave"):
                ...
                if new_status == "Fullført" and new_volunteer != "TBD":
                    ...
                    volunteer_email = pd.read_sql_query("SELECT email FROM volunteers WHERE name = ?", conn, params=(new_volunteer,))["email"].iloc[0]
